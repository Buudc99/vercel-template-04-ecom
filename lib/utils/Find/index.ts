 ...
 */
export const FindImageValues = (arrayData: any[]): string[] => {
  if (arrayData?.length > 0) {
    return arrayData
      .filter((item) => item.slug === "image" || item.slug === "thumbnaii")
      .map((item) => item.value);
  }
  return [];
};
 ...
 */
export const FilterDuplicateCategories = (categories: any[]): any[] => {
